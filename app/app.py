...
def predict():
    try:
        ...
        role = 1 if any(keyword.lower() in job_title_input.lower()
                        for keyword in manager_keywords) else 0

        # For 'City', since original data doesn't have it,
        # we randomly assigned it during training
        # For prediction, we need to provide a value.
        # Let's use a default (e.g., 0 for Non-Metro)
        # or randomly assign to keep consistency with training's random assignment
        # Randomly assign for prediction to match training's random assignment
        ...
        return render_template(
            'index.html',
            prediction_text=f'Invalid input. Error: {e}',
            education_levels=unique_education_levels,
            job_titles=unique_job_titles
        )
