...
if model is not None and scaler is not None and colleges_df is not None:
    with st.form("prediction_form"):
        ...
        # 4. Numerical Inputs
        previous_ctc = st.number_input(
            "Previous CTC (Annual)",
            min_value=0.0, step=1000.0, value=50000.0
        )
        previous_job_change = st.number_input(
        ...
        exp_years = st.number_input(
            "Years of Experience",
            min_value=0.0, max_value=50.0, step=0.1, value=2.0
        )
    ...
    if submitted:
        ...
        if college_input in tier1:
            college_tier = 3
        elif college_input in tier2:
            college_tier = 2
        elif college_input in tier3:
            college_tier = 1

        # City Mapping
        city_score = 0  # Default non-metro
        if city_input in metro:
            city_score = 1
    ...
    st.error("Model/Data files missing. Run 'train.py' locally first.")
